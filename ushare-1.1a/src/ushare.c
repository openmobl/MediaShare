...
static int http_port_to_listen = 8080;
static int port_to_open = 0;

int open_port(int open, int port);
#endif /* HAVE_WEBOS */
...
ushare_new (void)
{
  ...
  ut->ip = NULL;
#ifdef HAVE_WEBOS
  ut->port = 49152; /* Randomly attributed by libupnp */
#else
...
#ifdef HAVE_WEBOS
int
ushare_main(int argc, char **argv)
#else
int
...
PDL_bool open_port_result(PDL_ServiceParameters *params, void *user)               
{
    const char *result = PDL_GetParamJson(params);
    syslog(LOG_WARNING, "Opening Firewall result: %s", result);
    return PDL_TRUE;
}

int open_port(int open, int port)
{
    PDL_Err result = PDL_NOERROR;
    const char *open_payload = "{ \"subscribe\": true, \"rules\": [{\"protocol\":\"TCP\",\"destinationPort\":%d}, {\"protocol\":\"UDP\",\"destinationPort\":%d}] }";
    const char *close_payload = "{ \"subscribe\": true, \"rules\": [{\"protocol\":\"TCP\",\"operation\":\"delete\",\"destinationPort\":%d}, {\"protocol\":\"UDP\",\"operation\":\"delete\",\"destinationPort\":%d}] }";
    int port_str_len = strlen(close_payload) + 100;
    char *port_str = (char *)malloc(port_str_len);
    
    syslog(LOG_WARNING, "Opening Firewall");
    ...
    if (open) {
        snprintf(port_str, port_str_len, open_payload, port, port);
        port_to_open = port;
    } else {
        snprintf(port_str, port_str_len, close_payload, port, port);
        port_to_open = 0;
    }
    
    syslog(LOG_WARNING, "Payload: %s", port_str);
    result = PDL_ServiceCallWithCallback("palm://com.palm.firewall/control",
                port_str, open_port_result, NULL, PDL_FALSE);

    syslog(LOG_WARNING, "Result from service: %d", result);
    free(port_str);
    
    return (result == PDL_NOERROR) ? 0 : -1;
}

PDL_bool MediaShareIsReady(PDL_JSParameters *params)
{
    PDL_JSReply(params, "{ \"status\": \"ready\" }");
    return PDL_TRUE;
}

static pthread_t threadId = 0;
typedef struct MediaShareThreadArgs {
	char **arguments;
	int argCount;
} MediaShareThreadArgs;

void *MediaShareStartThread(void *args)
{
	MediaShareThreadArgs *threadArgs = (MediaShareThreadArgs *)args;
	int ret = 0;
	int i = 0;
	
	syslog(LOG_WARNING, "Thread starting");
	
	if (!threadArgs) {
		syslog(LOG_WARNING, "NULL arguments");
		pthread_exit(NULL);
	}
	syslog(LOG_WARNING, "Starting with argCount = %d", threadArgs->argCount);
	
	for (i = 0; i < threadArgs->argCount; i++) {
		syslog(LOG_WARNING, "ARG: %s", threadArgs->arguments[i]);
	}
	
	ret = ushare_main(threadArgs->argCount, threadArgs->arguments);
	if (ret) {
		syslog(LOG_WARNING, "Thread error %d", ret);
	}
...
PDL_bool MediaShareStart(PDL_JSParameters *params)
{
    PDL_bool status = PDL_FALSE;
    int ret = EXIT_FAILURE;
    int paramCount = PDL_GetNumJSParams(params) + 1;
    int i = 0;
	MediaShareThreadArgs *threadArgs = NULL;
    
    syslog(LOG_WARNING, "Starting up");

	if (threadId) {
		syslog(LOG_WARNING, "Running");
		
		PDL_JSReply(params, "{ \"status\": \"running\" }");
		return PDL_TRUE;
	}
	
	threadArgs = (MediaShareThreadArgs *)malloc(sizeof(MediaShareThreadArgs));
	if (!threadArgs) {
        PDL_JSReply(params, "{ \"status\": \"failed\", \"error\": \"ENOMEM\"}");
        return PDL_FALSE;
	}
	threadArgs->argCount = paramCount;
	
	threadArgs->arguments = (char **)malloc(sizeof(char*) * paramCount);
    if (!threadArgs->arguments) {
        PDL_JSReply(params, "{ \"status\": \"failed\", \"error\": \"ENOMEM\"}");
        return PDL_FALSE;
    }
    
    threadArgs->arguments[0] = "ushare";
    for (i = 0; i < paramCount; i++) {
        threadArgs->arguments[i+1] = strdup(PDL_GetJSParamString(params, i));
    }
	
	ret = pthread_create(&threadId, NULL,
              MediaShareStartThread, (void *)threadArgs);
    if (ret == 0) {
        syslog(LOG_WARNING, "Starting with thread id %d", (int)threadId);
        PDL_JSReply(params, "{ \"status\": \"started\" }");
        status = PDL_TRUE;
    } else {
        syslog(LOG_WARNING, "Failed to start up! %d %d", errno, ret);
        PDL_JSReply(params, "{ \"status\": \"failed\" }");
        status = PDL_FALSE;
    }
...
PDL_bool MediaShareStop(PDL_JSParameters *params)
{
    PDL_bool status = PDL_FALSE;
    int result = 0;
    
    syslog(LOG_WARNING, "Shutting down %d", (int)threadId);

	if (threadId)
		result = pthread_cancel(threadId);
    
	if (port_to_open)
		open_port(0, port_to_open);

    if (!result) {
		threadId = 0;
        syslog(LOG_WARNING, "Shut down");
        PDL_JSReply(params, "{ \"status\": \"stopped\" }");
        status = PDL_TRUE;
    } else {
        syslog(LOG_WARNING, "Failed to shut down! %d", result);
        PDL_JSReply(params, "{ \"status\": \"failed\" }");
        status = PDL_FALSE;
    }
...
PDL_bool MediaShareReload(PDL_JSParameters *params)
{
    PDL_bool status = PDL_FALSE;
    int result = 0;
    
    syslog(LOG_WARNING, "Reloading %d", (int)threadId);
    
    //result = kill(pid, SIGHUP);

    if (!result) {
        syslog(LOG_WARNING, "Process reloaded");
        PDL_JSReply(params, "{ \"status\": \"reloaded\" }");
        status = PDL_TRUE;
    } else {
        syslog(LOG_WARNING, "Failed to reload! %d", errno);
        PDL_JSReply(params, "{ \"status\": \"failed\" }");
        status = PDL_FALSE;
    }
...
int AppStart(void)
{
    ...
    err = PDL_RegisterJSHandler("start", MediaShareStart);
    syslog(LOG_WARNING, "RegisterFunction: start, %d\n", err);
    
    err = PDL_RegisterJSHandler("stop", MediaShareStop);
    syslog(LOG_WARNING, "RegisterFunction: stop, %d\n", err);
    
    err = PDL_RegisterJSHandler("reload", MediaShareReload);
    syslog(LOG_WARNING, "RegisterFunction: reload, %d\n", err);
    
    PDL_JSRegistrationComplete();
    
    /* Indicate that we are ready to run. */
    syslog(LOG_WARNING, "Indicating ready");
...
int AppStop(void)
{
	...
	PDL_UnregisterServiceCallback(open_port_result);

    PDL_Quit();
    return 0;
}

int AppEventLoop(void)
{
    SDL_Event event;
...
int main(int argc, char *argv[])
{
    ...
    openlog("MediaShare", LOG_PID, LOG_USER);
    
    if (SDL_Init(SDL_INIT_VIDEO)) {
        syslog(LOG_WARNING, "SDL Init FAILED");
        return -1;
    }
